...
$(function() {
  ...
  function fnc(){
    ...
    img_send('#test3',"#demo3")
    img_send('#test4',"#demo4")
    img_send('#test5',"#demo5")
    tuo_img("127.0.0.1")  
  }
  //封装分页layui
...
dom3.click(function(){
  $(".part1").html(str3);
  $(this).addClass('borderBlue').siblings().removeClass('borderBlue');
  getAjax("GET","/builder/get_builder_own_demand/",{page:1},'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNTY3NzY4MTk0LCJqdGkiOiJkOWUwN2Y2ZTUxNWE0ZDg2YjE5Y2U3MWZiYzczNmMyZSIsInVzZXJfaWQiOjE2LCJ1c2VybmFtZSI6Imhvd3RpbWUiLCJyb2xlIjoxfQ.HgpvNaSiVYCgbZRGrPaU7D-_SGNQ0ipjNqiIgWKVG2A').then(res=>{
    console.log(res);
    let readyPro=``
    ...
    for(let i=0;i<res[0].data.results.length;i++){
      ...
      <div class="sc-j">
          <div class="d-flex justify-content-around">
              ...
              <p>${res[0].data.results[i].demand_num}</p>
              <p>${res[0].data.results[i].jobs_salary}</p>
...
// tabClick(dom2,$(".part1"),str2);
// tabClick(dom3,$(".part1"),str3);
// tabClick(dom4,$(".part1"),str4);
// tabClick(dom5,$(".part1"),str5);
// tabClick(dom6,$(".part1"),str6);
//建筑商字段
...
function issue(){
  ...
  var demand_num =$(".demand_num");
  var jobs_salary = $(".jobs_salary");
  ...
  $("body").on("click",".btn-mess2",function(){
    ...
    Promise.all([
      $.ajax({ 
        url: 'http://132.232.6.121:8000/builder/builder_demand/',
        type: 'POST',
        data:{
          project_state:$('.project_state input:checked').val(),
          // project_name:project_name.val(),
          ...
          // jobs_salary:jobs_salary.val(),
          // demand_num:demand_num.val(),
          demand_job_id:$(".demand_job_id option:selected").val(),
 ...
 function myPutAjax(form){
  getAjax("post","/builder/builder_info/",form,sessionStorage.access).then(
    resolve=>{
